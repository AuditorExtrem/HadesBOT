...
# ====== FICHAS SYSTEM ======
FICHAS_CANAL_ID = 1390134033387618474  # Canal de testes para as fichas
...
numero_inicial_ficha = carregar_numero_ficha()
...
async def iniciar_formulario(bot, interaction, idioma):
    ...
    respostas = {}
    canal = await bot.fetch_channel(FICHAS_CANAL_ID)
    def check(m):
    ...
    # busca o maior número já usado em ambos arquivos
    all_nums = []
    for lang in ["pt", "en"]:
        try:
    ...
    # Novo: editar a mensagem anterior se existir, senão enviar uma nova
    mensagem_id = None
