...
FICHAS_CANAL_HADES2_ID = 1390146714417102938  # Substitua pelo ID do canal Hades2
...
def carregar_ficha(user_id, idioma, guilda):
    ...
    try:
        with open(arquivo, "r", encoding="utf-8") as f:
            ...
            # Só retorna se for da guilda correta
...
async def slash_ficha_hades2(interaction: discord.Interaction):
    view = MenuIdioma(bot, interaction, FICHAS_CANAL_HADES2_ID, "hades2")
    msg = (
        "📄 Clique abaixo para escolher o idioma.\n"
        "⚠️ **Você só pode ter UMA ficha registrada. Preencher de novo irá editar sua ficha!**\n"
...
async def iniciar_formulario(bot, interaction, idioma, canal_id, nome_guilda):
    ...
    # Numeração separada por guilda!
    all_nums = []
    for lang in ["pt", "en"]:
        try:
            with open(f"fichas_{lang}.json", "r", encoding="utf-8") as f:
                all_nums += [x["numero"] for x in json.load(f).values() if "numero" in x and x.get("guilda") == nome_guilda]
        except:
            pass
    numero_ficha = ficha_existente["numero"] if ficha_existente and "numero" in ficha_existente else max([carregar_numero_ficha(nome_guilda)] + all_nums, default=carregar_numero_ficha(nome_guilda))
    ...
    embed.add_field(name="🏰 Guilda", value="Hades" if nome_guilda=="hades" else "Hades 2", inline=True)
