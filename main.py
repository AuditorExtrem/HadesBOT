...
@app_commands.default_permissions(administrator=True)
@app_commands.describe(numero="Número da ficha", idioma="pt/en/es", campo="Campo a corrigir", valor="Novo valor")
async def editar_ficha(interaction: discord.Interaction, numero: int, idioma: str, campo: str, valor: str):
    idioma = idioma.lower()
    ...
    try:
        ...
        await interaction.response.send_message("❌ Nenhuma ficha encontrada nesse idioma.", ephemeral=True)
        return
    for uid, ficha in todas.items():
        if ficha.get("numero") == numero:
            ...
            with open(arquivo, "w", encoding="utf-8") as f:
                json.dump(todas, f, indent=4, ensure_ascii=False)
            await interaction.response.send_message(f"✅ Ficha número {numero} atualizada! Campo {campo} corrigido para: {valor}", ephemeral=True)
            return
    await interaction.response.send_message("❌ Ficha não encontrada.", ephemeral=True)

# ----------- COMANDOS SLASH SERVIDORES --------------
