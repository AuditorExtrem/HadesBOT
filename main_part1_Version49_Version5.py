...
async def enviar_ficha_no_canal(bot, user, idioma, ficha, nome_guilda, canal_id):
    ...
    # Buscar membro por ID e mostrar mention (ex: @Parzival)
    discord_id = ficha.get("discord", None)
    discord_str = "-"
    canal = bot.get_channel(canal_id)
    member = None
    if canal:  # tenta pegar o membro da guilda do canal
        guild = canal.guild
        if guild and discord_id and str(discord_id).isdigit():
            member = guild.get_member(int(discord_id))
    if member:
        discord_str = member.mention
    elif discord_id and str(discord_id).isdigit():
        discord_str = f"<@{discord_id}>"
    elif discord_id:
        discord_str = str(discord_id)
    embed = discord.Embed(
...
async def ver_ficha(
    ...
    for uid, ficha in fichas.items():
        if ficha.get("numero") == numero:
            ...
            # Buscar membro pelo ID salvo na ficha para mostrar mention
            ...
            if member:
                ...
                discord_str = discord_id if discord_id else "-"
            embed = discord.Embed(
                title=f"🌌 Ficha de Jogador #{ficha['numero']} – Arise Crossover {bandeira} 🌌",
                color=discord.Color.purple()
            )
...
class EditarFichaDiscordModal(ui.Modal, title="Editar Ficha - Discord"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Buscar o member para mostrar @nome na resposta
        ...
        if member:
            ...
            display = f"<@{user_id}>"
        await interaction.response.send_message(
            f"✅ Ficha número {numero} da guilda {guilda} atualizada!\nCampo **{campo}** corrigido para: {display}",
            ephemeral=True
        )
